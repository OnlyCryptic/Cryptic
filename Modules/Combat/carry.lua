...
return function(Tab, UI)
    ...
    local function RemoveHeightUI()
        ...
        if carryGui then carryGui:Destroy() carryGui = nil end
    end

    Tab:AddToggle(T("combat.carry.label"), function(active)
        isCarrying = active
        ...
        if active then
            ...
            if myTorso and targetTorso then
                ...
                if success and not canCollide then
                    isCarrying = false
                    Notify(T("combat.carry.no_collision"))
                    return
                end
            end
