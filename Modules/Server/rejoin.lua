...
return function(Tab, UI)
    ...
    -- 1. Rejoin
    ActionBtn("🔄", T("server.rejoin.title"), T("server.rejoin.sub"), Color3.fromRGB(0, 180, 255), function()
        Notify("Cryptic Hub", "⏳ " .. T("server.rejoin.progress"))
        task.wait(0.8)
        pcall(function()
    ...
    -- 2. Server Hop
    ActionBtn("🌐", T("server.hop.title"), T("server.hop.sub"), Color3.fromRGB(150, 80, 255), function()
        Notify("Cryptic Hub", "🔍 " .. T("server.hop.searching"))
        ...
        if ok and #validServers > 0 then
            ...
            Notify("Cryptic Hub", "🚀 " .. T("server.hop.found"))
            task.wait(0.5)
            pcall(function() TeleportService:TeleportToPlaceInstance(game.PlaceId, pick, player) end)
        elseif ok then
            Notify("Cryptic Hub", "⚠️ " .. T("server.hop.none"))
        else
            Notify("Cryptic Hub", "❌ " .. T("server.hop.fail"))
        end
    end)
end
