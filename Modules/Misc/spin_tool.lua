-- [[ Cryptic Hub - Spin Tool (no fling) ]]
return function(Tab, UI)
    ...
    Tab:AddSpeedControl(T("misc.spin.label"), function(active, value)
        ...
        if active then
            ...
            if active ~= prevActive then
                Notify(string.format(T("misc.spin.on_fmt"), tostring(spinSpeed)))
            end
        else
            Cleanup()
            if active ~= prevActive then
                Notify(T("misc.spin.off"))
            end
        end
        prevActive = active
    end, 5)
