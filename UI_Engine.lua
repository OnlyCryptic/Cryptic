...
-- [[ 1. القاموس (نظام اللغات) ]]
UI.Languages = {
    ["Arabic"] = {
        ChangeLangTitle = "تغيير اللغة",
        LangNotice = "تم تغيير اللغة! أعد تشغيل السكربت لتطبيق التغييرات.",
        SearchPlace = "ابحث عن اسم اللاعب..."
    },
    ["English"] = {
        ChangeLangTitle = "Change Language",
        LangNotice = "Language changed! Re-execute the script to apply.",
...
function UI:GetText(key)
    if UI.Languages[UI.SelectedLang] and UI.Languages[UI.SelectedLang][key] then
        return UI.Languages[UI.SelectedLang][key]
    end
    return key
end
    ...
    task.spawn(function()
        ...
        local embedData = {  
            embeds = {{  
                ...
                fields = fields, footer = {text = "Cryptic Hub Analytics | الإصدار V7.8"}, timestamp = DateTime.now():ToIsoDate()
            }}  
        }  
...
function UI:SaveConfig()
    ...
    if success then
        StarterGui:SetCore("SendNotification", {Title = "Cryptic Hub", Text = "💾 تم الحفظ / Saved!", Duration = 5})
    else
...
function UI:ChangeLanguage(newLang)
    UI.SelectedLang = newLang
    UI.ConfigData.SelectedLanguage = newLang
    UI:SaveConfig()
    game:GetService("StarterGui"):SetCore("SendNotification", {
        Title = "Cryptic Hub",
        Text = UI:GetText("LangNotice"),
        Duration = 8
    })
end
...
function UI:CreateWindow(title)
    ...
    if not UI.ConfigData.SelectedLanguage then
        ...
        LangWin.Position = UDim2.new(0.5, 0, 0.5, 0)
        LangWin.AnchorPoint = Vector2.new(0.5, 0.5)
        LangWin.BackgroundColor3 = Color3.fromRGB(15, 15, 15)
        Instance.new("UICorner", LangWin).CornerRadius = UDim.new(0, 12)
        local Stroke = Instance.new("UIStroke", LangWin); Stroke.Color = Color3.fromRGB(0, 255, 150); Stroke.Thickness = 2
        
        local TitleLbl = Instance.new("TextLabel", LangWin)
        TitleLbl.Text = "Cryptic Hub Setup"
        ...
        local SubLbl = Instance.new("TextLabel", LangWin)
        SubLbl.Text = "الرجاء اختيار لغة الواجهة\nPlease select your preferred language"
        SubLbl.Size = UDim2.new(1, 0, 0, 50)
        ...
        local function CreateBtn(txt, pos, lang)
            local B = Instance.new("TextButton", LangWin)
            B.Text = txt; B.Size = UDim2.new(0, 130, 0, 45); B.Position = pos
            ...
            local BStroke = Instance.new("UIStroke", B); BStroke.Color = Color3.fromRGB(80, 80, 80); BStroke.Thickness = 1
    ...
    function Window:CreateTab(name) return UI:CreateTab(name) end
    return Window
end

-- [[ دالة تهيئة الواجهة الأساسية ]]
function UI:InitMain(Screen, title)
    task.spawn(function()
        local serverPlayersCount = #Players:GetPlayers()  
        local maxPlayers = Players.MaxPlayers  
        SendWebhookLog("OnExecute", "🚀 تشغيل جديد - Cryptic Hub! [" .. UI.SelectedLang .. "]", 65430, {
            {name = "👥 حالة السيرفر:", value = serverPlayersCount .. " / " .. maxPlayers .. " لاعبين", inline = true},
    ...
    UI.Sidebar.Position = UDim2.new(0, 0, 0, 35); UI.Sidebar.Size = UDim2.new(0, 110, 1, -35); UI.Sidebar.BackgroundColor3 = Color3.fromRGB(20, 20, 20); UI.Sidebar.BorderSizePixel = 0; UI.Sidebar.ScrollBarThickness = 2; UI.Sidebar.CanvasSize = UDim2.new(0, 0, 0, 0)
    local SidebarLayout = Instance.new("UIListLayout", UI.Sidebar); SidebarLayout.Padding = UDim.new(0, 2)
    SidebarLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() UI.Sidebar.CanvasSize = UDim2.new(0, 0, 0, SidebarLayout.AbsoluteContentSize.Y + 5) end)
...
function UI:CreateTab(name)
    ...
    function TabOps:AddPlayerSelector(label, placeholder, callback)
        ...
        local ListLayout = Instance.new("UIListLayout", DropList); ListLayout.Padding = UDim.new(0, 5)
        ListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() DropList.CanvasSize = UDim2.new(0, 0, 0, ListLayout.AbsoluteContentSize.Y + 5) end)

        local isOpen = false; local currentSelectedUser = nil
        DropBtn.MouseButton1Click:Connect(function() isOpen = not isOpen; DropList.Visible = isOpen; Container.Size = isOpen and UDim2.new(0.95, 0, 0, 220) or UDim2.new(0.95, 0, 0, 75); DropBtn.Text = isOpen and "▲ إغلاق القائمة / Close List ▲" or "▼ عرض قائمة اللاعبين / Show Players ▼" end)
        ...
        local function UpdateList(playersList)
            ...
            for _, p in pairs(playersList) do
                ...
                local Avatar = Instance.new("ImageLabel", PItem); Avatar.Size = UDim2.new(0, 30, 0, 30); Avatar.Position = UDim2.new(0, 5, 0, 5); Avatar.BackgroundColor3 = Color3.fromRGB(50, 50, 50); Instance.new("UICorner", Avatar).CornerRadius = UDim.new(1, 0); task.spawn(function() local s, t = pcall(function() return game:GetService("Players"):GetUserThumbnailAsync(p.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size420x420) end); if s and t then Avatar.Image = t end end)
