-- [[ Cryptic Hub - محرك الواجهة المطور V2.5 ]]
-- المطور: Arwa | إصلاح شامل لنظام السحب (Mobile Drag Fix)
-- التحديث: دعم اللمس لـ Redmi Note 10s والفواصل المنظمة
...
function UI:CreateWindow(title)
    ...
    -- 1. زر C العائم (قابل للسحب على الجوال)
    ...
    -- منطق سحب زر C المخصص للمس
    local draggingC, dragStartC, startPosC
    OpenBtn.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch then
            draggingC = true; dragStartC = input.Position; startPosC = OpenBtn.Position
        end
    end)
    UserInputService.InputChanged:Connect(function(input)
        if draggingC and input.UserInputType == Enum.UserInputType.Touch then
    ...
    UserInputService.InputEnded:Connect(function(input) if input.UserInputType == Enum.UserInputType.Touch then draggingC = false end end)
    ...
    -- سحب الواجهة الرئيسية (Mobile Fix)
    local dragWin, dragStartWin, startPosWin
    TitleBar.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch then
            dragWin = true; dragStartWin = input.Position; startPosWin = Main.Position
        end
    end)
    UserInputService.InputChanged:Connect(function(input)
        if dragWin and input.UserInputType == Enum.UserInputType.Touch then
    ...
    UserInputService.InputEnded:Connect(function(input) if input.UserInputType == Enum.UserInputType.Touch then dragWin = false end end)
    ...
    function Window:CreateTab(name)
        ...
        function TabOps:AddInput(label, placeholder, callback)
            ...
            Inp.Focused:Connect(function() ignore = true; Inp.Text = ""; ignore = false end)
