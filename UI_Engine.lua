-- [[ Cryptic Hub - محرك الواجهة المطور V6.8 (النسخة المتكاملة الآمنة) ]]
-- المطور: يامي (Yami) | التحديث: جميع الدوال مدمجة + زر فتح احترافي مصغر
...
function UI:SaveConfig()
    local success = pcall(function() writefile(ConfigFile, HttpService:JSONEncode(UI.ConfigData)) end)
    local StarterGui = game:GetService("StarterGui")
    if success then StarterGui:SetCore("SendNotification", {Title = "Cryptic Hub", Text = "💾 تم حفظ الإعدادات بنجاح!", Duration = 5})
    else StarterGui:SetCore("SendNotification", {Title = "⚠️ خطأ", Text = "فشل الحفظ: المشغل لا يدعم الميزة.", Duration = 5}) end
end
...
function UI:CreateWindow(title)
    ...
    -- [[ الزر الجديد: صغير، فخم، بدون إيموجي، وشكل كبسولة ]]
    local OpenBtn = Instance.new("TextButton", Screen)
    OpenBtn.Size = UDim2.new(0, 80, 0, 32)
    OpenBtn.Position = UDim2.new(0, 15, 0.5, -16)
    OpenBtn.Visible = false
    OpenBtn.Text = "Cryptic"
    OpenBtn.BackgroundColor3 = Color3.fromRGB(25, 25, 25)
    OpenBtn.TextColor3 = Color3.fromRGB(0, 255, 150)
    OpenBtn.Font = Enum.Font.GothamBold
    OpenBtn.TextSize = 14
    Instance.new("UICorner", OpenBtn).CornerRadius = UDim.new(1, 0) -- يخليه كبسولة بيضاوية
    ...
    ConfirmLbl.Size = UDim2.new(1, 0, 0, 50); ConfirmLbl.Position = UDim2.new(0, 0, 0, 10); ConfirmLbl.BackgroundTransparency = 1; ConfirmLbl.Text = "هل أنت متأكد من الإغلاق؟"; ConfirmLbl.TextColor3 = Color3.new(1,1,1); ConfirmLbl.Font = Enum.Font.GothamBold; ConfirmLbl.TextSize = 14; ConfirmLbl.ZIndex = 12
    ...
    function Window:CreateTab(name)
        ...
        -- دعم استدعاء الدوال المستقبلية من UI_2
        setmetatable(TabOps, {
        ...
        function TabOps:AddLabel(t) local R = Instance.new("Frame", Page); R.LayoutOrder = #Page:GetChildren() + 1; R.Size = UDim2.new(0.98,0,0,35); R.BackgroundColor3 = Color3.fromRGB(25,25,25); Instance.new("UICorner",R); local L = Instance.new("TextLabel",R); L.Text = t; L.Size = UDim2.new(1,-10,1,0); L.TextColor3 = Color3.fromRGB(0, 255, 150); L.BackgroundTransparency = 1; L.TextXAlignment = Enum.TextXAlignment.Right; L.Font = Enum.Font.Gotham; L.TextSize = 14; return {SetText=function(nt) L.Text=nt end} end
        function TabOps:AddParagraph(text) local Lbl = Instance.new("TextLabel", Page); Lbl.LayoutOrder = #Page:GetChildren() + 1; Lbl.Size = UDim2.new(0.95, 0, 0, 0); Lbl.AutomaticSize = Enum.AutomaticSize.Y; Lbl.TextWrapped = true; Lbl.Text = text; Lbl.TextColor3 = Color3.fromRGB(170, 170, 170); Lbl.BackgroundTransparency = 1; Lbl.TextXAlignment = Enum.TextXAlignment.Right; Lbl.Font = Enum.Font.Gotham; Lbl.TextSize = 13 end
        ...
        function TabOps:AddSpeedControl(label, callback, default)
            ...
            local function update() Tgl.BackgroundColor3 = active and Color3.fromRGB(0, 150, 255) or Color3.fromRGB(60, 60, 60); local val = tonumber(Inp.Text) or tonumber(startVal); UI.ConfigData[configKey] = {active = active, val = val}; callback(active, val) end
        ...
        function TabOps:AddTimedToggle(label, callback)
            ...
            local B = Instance.new("TextButton", R); B.Size = UDim2.new(0, 45, 0, 22); B.Position = UDim2.new(1, -55, 0.5, -11); B.Text = ""; B.BackgroundColor3 = Color3.fromRGB(60, 60, 60); Instance.new("UICorner", B).CornerRadius = UDim.new(1, 0)
            local Lbl = Instance.new("TextLabel", R); Lbl.Text = label; Lbl.Size = UDim2.new(0.7, 0, 1, 0); Lbl.Position = UDim2.new(0.05, 0, 0, 0); Lbl.TextColor3 = Color3.new(1, 1, 1); Lbl.BackgroundTransparency = 1; Lbl.TextXAlignment = Enum.TextXAlignment.Right; Lbl.Font = Enum.Font.Gotham; Lbl.TextSize = 14; local isRunning = false
