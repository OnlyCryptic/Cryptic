-- [[ Cryptic Hub - محرك الواجهة المطور V2.4 ]]
-- التحديث: إصلاح نظام البحث والوصول المباشر لصندوق النص
...
function UI:CreateWindow(title)
    ...
    function Window:CreateTab(name)
        ...
        function TabOps:AddLine()
            local Line = Instance.new("Frame", Page); Line.Size = UDim2.new(0.95, 0, 0, 1); Line.BackgroundColor3 = Color3.fromRGB(50, 50, 50); Line.BackgroundTransparency = 0.5; Line.BorderSizePixel = 0
        end
        ...
        function TabOps:AddInput(label, placeholder, callback)
            ...
            Inp:GetPropertyChangedSignal("Text"):Connect(function() if not ignoreChange then callback(Inp.Text) end end)
            Inp.Focused:Connect(function() ignoreChange = true; Inp.Text = ""; ignoreChange = false end)

            -- نرجع الصندوق نفسه (TextBox) لكي نستخدمه في حدث FocusLost
            return { SetText = function(t) ignoreChange = true; Inp.Text = t; ignoreChange = false end, TextBox = Inp }
        end
