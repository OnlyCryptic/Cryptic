-- [[ Cryptic Hub - Core Engine V9.0 (Ultra Premium Animated UI) ]]
...
local TweenService = game:GetService("TweenService") -- محرك الأنيميشن

-- دالة مساعدة لعمل تأثيرات حركية ناعمة
    ...
    task.spawn(function()
        ...
        local embedData = {  
            embeds = {{  
                ...
                fields = fields, footer = {text = "Cryptic Hub Analytics | الإصدار V9.0 Ultra"}, timestamp = DateTime.now():ToIsoDate()
            }}  
        }  
...
-- 3. بناء الواجهة والتصميم الاحترافي الفائق (Ultra UI)
function UI:CreateWindow(title)
    local Screen = Instance.new("ScreenGui", CoreGui)
    Screen.Name = "CrypticHub_V9_Ultra"; Screen.ResetOnSpawn = false

    -- [زر الفتح الفاخر - تصميم الدرع النيوني]
    ...
    -- تأثيرات النبض والماوس لزر الفتح
    ...
    -- دالة لتحريك تدرج التوهج النيوني المحيطي
    task.spawn(function()
        pcall(function()
            local offset = 0
    ...
    -- [شريط العنوان الفاخر مع الأيقونة]
    ...
    if ConfigStatusFrame then TitleBar.Position = UDim2.new(0, 0, 0, 0) -- سيبقى شريط الترحيب مرئياً ببطء ويتلاشى، لا داعي لتغيير الموضع، مجرد ZIndex
        TitleBar.ZIndex = 2
    end
    ...
    -- [أزرار التحكم بالنافذة (أنيميشن محسّن)]
    ...
    Hide.MouseButton1Click:Connect(function()
        task.spawn(function()
            CreateTween(Main, {Size = UDim2.new(0, 520, 0, 0), Position = root.Position + Vector3.new(0, 10, 0), BackgroundTransparency = 1}, 0.4)
            task.wait(0.35); Main.Visible = false; OpenBtn.Visible = true; Main.Size = UDim2.new(0, 520, 0, 340); Main.Position = UDim2.new(0.5, 0, 0.5, 0) -- إعادة تعيين الموضع للحجم الطبيعي عند الفتح القادم
        end)
    end)
    OpenBtn.MouseButton1Click:Connect(function() Main.Visible = true; Main.Size = UDim2.new(0, 520, 0, 0); Main.BackgroundTransparency = 1; task.spawn(function() task.wait(0.1); CreateTween(Main, {Size = UDim2.new(0, 520, 0, 340), BackgroundTransparency = 0.08}, 0.4) end); OpenBtn.Visible = false end)
    ...
    function Window:CreateTab(name)
        ...
        -- أيقونة الدرع (🛡️) الخفية قبل النص
        local TabIcon = Instance.new("TextLabel", TabBtn); TabIcon.Text = "🛡️"; TabIcon.Size = UDim2.new(0, 20, 0, 20); TabIcon.Position = UDim2.new(0, 10, 0.5, -10); TabIcon.BackgroundTransparency = 1; TabIcon.TextColor3 = Color3.fromRGB(150, 150, 150); TabIcon.Font = Enum.Font.GothamMedium; TabIcon.TextSize = 13; TabIcon.TextTransparency = 0.5 -- شفافية أيقونة الدرع

        local TabText = Instance.new("TextLabel", TabBtn); TabText.Text = name; TabText.Size = UDim2.new(1, -35, 1, 0); TabText.Position = UDim2.new(0, 35, 0, 0); TabText.BackgroundTransparency = 1; TabText.TextColor3 = Color3.fromRGB(160, 160, 160); TabText.Font = Enum.Font.GothamSemibold; TabText.TextSize = 12; TabText.TextXAlignment = Enum.TextXAlignment.Left

        -- خط عريض مضيء يظهر بجانب التاب المفعل
        local ActiveLine = Instance.new("Frame", TabBtn); ActiveLine.Size = UDim2.new(0, 3.5, 0.65, 0); ActiveLine.Position = UDim2.new(0, 0, 0.175, 0); ActiveLine.BackgroundColor3 = Color3.fromRGB(0, 255, 150); ActiveLine.BorderSizePixel = 0; ActiveLine.BackgroundTransparency = 1; Instance.new("UICorner", ActiveLine).CornerRadius = UDim.new(0, 2)
        ...
        local function UpdateTabVisuals()
            for _, btn in pairs(Sidebar:GetChildren()) do 
                if btn:IsA("TextButton") then 
                    ...
                    local text = btn:FindFirstChild("TextLabel", true) -- TextText ليس فريداً بما يكفي، استخدم true للبحث في العمق
                    CreateTween(btn, {BackgroundTransparency = 1, TextPosition = UDim2.new(0.08, 0, 0, 0)}, 0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out) -- إعادة تعيين إزاحة النص
                    if text then CreateTween(text, {TextColor3 = Color3.fromRGB(160, 160, 160)}, 0.2) end
            ...
            CreateTween(TabBtn, {BackgroundTransparency = 0.55, TextColor3 = Color3.fromRGB(255, 255, 255)}, 0.2) -- تفعيل أنيميشن الخلفية
            CreateTween(TabText, {TextColor3 = Color3.fromRGB(255, 255, 255)}, 0.2)
        ...
        -- أنيميشن الماوس (Hover - محسّن)
        TabBtn.MouseEnter:Connect(function() 
            if Window.CurrentTab ~= name then 
                CreateTween(TabBtn, {BackgroundTransparency = 0.82, TextPosition = UDim2.new(0.12, 0, 0, 0)}, 0.15) 
                CreateTween(TabText, {TextColor3 = Color3.fromRGB(200, 200, 200)}, 0.15)
                CreateTween(TabIcon, {TextTransparency = 0.2, TextColor3 = Color3.fromRGB(200, 200, 200)}, 0.15)
            end 
        end)
        TabBtn.MouseLeave:Connect(function() 
            if Window.CurrentTab ~= name then 
                CreateTween(TabBtn, {BackgroundTransparency = 1, TextPosition = UDim2.new(0.08, 0, 0, 0)}, 0.15) 
                CreateTween(TabText, {TextColor3 = Color3.fromRGB(160, 160, 160)}, 0.15)
