-- [[ Cryptic Hub - Element: ExclusiveToggle ]]
-- توغل حصري — لما يُفعَّل يطفئ باقي التوغلات في نفس المجموعة تلقائياً

return function(TabOps, label, groupRef, callback)
    ...
    -- أيقونة صغيرة تدل على أنه حصري
    local ExcIcon = Instance.new("TextLabel", R)
    ExcIcon.Text                   = "◈"
    ExcIcon.Size                   = UDim2.new(0, 14, 0, 14)
    ExcIcon.Position               = UDim2.new(0, 10, 0.5, -7)
    ExcIcon.TextColor3             = Color3.fromRGB(0, 200, 120)
    ExcIcon.BackgroundTransparency = 1
    ExcIcon.Font                   = Enum.Font.GothamBold
    ExcIcon.TextSize               = 10
    ...
    local function setState(state, isClick)
        isActive = state
        if isActive then
            -- أطفئ جميع التوغلات الأخرى في المجموعة
            if groupRef then
                for _, other in ipairs(groupRef) do
                    if other ~= self then
            ...
            Tw(ExcIcon, {TextColor3 = Color3.fromRGB(0, 255, 150)}, 0.18)
        else
    ...
    if isActive then
        ...
        ExcIcon.TextColor3     = Color3.fromRGB(0, 255, 150)
        task.spawn(function() task.wait(1.5); pcall(callback, isActive) end)
    end

    self.SetState = function(_, state) setState(state, false) end

    -- سجّل نفسك في المجموعة
    if groupRef then
        table.insert(groupRef, self)
    end
